...
fetch("./js/env.json")
  ...
  .then(async (data) => {
      ...
      if (!user) {
        user = await Moralis.authenticate({
          ...
          .then(function (user) {
            ...
            document.getElementById("wallet-address").innerHTML = user.get(
              "ethAddress"
            );
          })
    ...
    async function upload() {
      ...
      const unlockableContent = document.getElementById("unlockable-content")
        .checked;
      const sensitiveContent = document.getElementById("sensitive-content")
        .checked;
      const confirmAuthority = document.getElementById("confirm-authority")
        .checked;
      ...
      const nftFile = new Moralis.File("file.json", {
      ...
      await Moralis.executeFunction(options).then(async (result) => {
        console.log(result);
        ...
        mints.set("image", file.ipfs());
        mints.set("transaction", result.hash);
        mints.set("tokenId", "");
        mints.set("owner", "");
        mints.set("transaction", result.hash);
        await mints.save().then(
          (mint) => {
            console.log(mint.id);
          },
          (error) => {
    ...
    document.getElementById("btn-pr").onclick = pr;
  })
